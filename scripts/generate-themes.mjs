...
const MONACO_COLOR_MAP = new Set([
  'foreground',
  'errorForeground',
...
const convertVsCodeThemeToMonaco = vsCodeTheme => {
  ...
  Object.keys(colors).forEach(colorKey => {
    ...
    if (MONACO_COLOR_MAP.has(colorKey) && color) {
...
const processThemes = async () => {
  try {
    ...
    // Validate VS Code theme file names
    vsCodeThemeFiles.forEach(file => {
      if (!file.endsWith('.json')) {
        throw new Error(
          `\`${file}\` file name has no proper .json extension. Only .json files are allowed in the themes directory.`,
        );
      }

      if (file.includes(' ')) {
        throw new Error(
          `\`${file}\` file name contains spaces. File names should not contain spaces. Please use hyphens instead.`,
        );
      }

      if (file !== file.toLowerCase()) {
        throw new Error(
          `\`${file}\` file name contains uppercase letters. File names should be lowercase. Please use lowercase letters only.`,
        );
      }
    });
